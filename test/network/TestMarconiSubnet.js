const MarconiSubnet = artifacts.require("MarconiSubnet.sol");
const NetworkManager = artifacts.require("NetworkManager.sol");

contract('MarconiSubnet', function ([account0, account1]) {
    ...
    beforeEach('setup contract for each test', async function () {
        ...
        network = await MarconiSubnet.new(0, account0);
    ...
    it('check empty network', async function () {
        assert.equal(await network.getPeerCount(), 0);
        let jsonExpected = '{ "networkId": "0", "admin": "' + admin +'", "active": true, "peers": [] }';;
    ...
    it('get data JSON', async function () {
        await addPeersOneAndTwo();
        let jsonExpected = '{ "networkId": "0", "admin": "' + admin + '", "active": true, "peers": [{"pubKeyHash": "abc123", "macHash": "", "ip": "10.27.16.10/24", "neighbors": ["abc123", "456xyz"]}, {"pubKeyHash": "456xyz", "macHash": "", "ip": "10.27.16.11/24", "neighbors": ["abc123", "456xyz"]}] }';
        assert.equal(await network.getDataJSON(), jsonExpected);
    });
    ...
    async function addPeerOne() {
        ...
        let jsonExpected = '{ "networkId": "0", "admin": "' + admin + '", "active": true, "peers": [{"pubKeyHash": "abc123", "macHash": "", "ip": "10.27.16.10/24", "neighbors": ["abc123]}] }';
        assert.equal(await network.getDataJSON(), jsonExpected);
