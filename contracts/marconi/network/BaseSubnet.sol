...
/*
** Base contract encapsulating the common logic of managing a sub-network such as add/remove of peers.
**
** This is the building block for the Marconi Subnet contract and other variations of Subnets in the future.
*/
contract BaseSubnet {
    ...
    string constant public IP_PREFIX = "10.10.10.";  // the prefix for peer's DHCP IPs
    ...
    */
    constructor (uint256 _id, address _admin) public {
        id = _id;
        admin = _admin;
        active = true;
    }
    ...
    function addPeer(string memory _pubKeyHash) isActive public returns (bool) {
        ...
        require(peerCount < getMaxPeerCapacity()); // check network is not full
        ...
        string memory _hostIP = Util.uintToStr(_index + getIpOffset());
        peer.ip = getIpPrefix().toSlice().concat(_hostIP.toSlice());
        peer.ip = peer.ip.toSlice().concat(getIpSuffix().toSlice());
     ...
     */
    function transferNetworkAdmin(address _newAdmin) public {
        require(admin == msg.sender);       // only current admin may change ownership
    ...
    function getReleaseVersion() public pure returns(uint256) {
